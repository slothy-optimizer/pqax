...
// Derived, with minor modifications, from public domain implementation
...
void keccak_f1600_x1_scalar_C( uint64_t state[KECCAK_F1600_X1_STATE_SIZE_UINT64] )
{
    ...
    for( int round = 0; round < KECCAK_F1600_ROUNDS; round += 2 )
    {
        //    prepareTheta
...
void keccak_f1600_x2_scalar_C(uint64_t state[2*25])
{
    ...
    for(size_t i=0;i<25;i++){
        state1[i] = state[2*i+0];
        state2[i] = state[2*i+1];
    }

    keccak_f1600_x1_scalar_C(state1);
    keccak_f1600_x1_scalar_C(state2);

    // interleave
    for(size_t i=0;i<25;i++){
        state[2*i+0] = state1[i];
        state[2*i+1] = state2[i];
    }
}
