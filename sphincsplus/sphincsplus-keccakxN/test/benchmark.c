...
int main()
{
    ...
    unsigned char addr[SPX_ADDR_BYTES*KECCAK_WAY];
    unsigned char wots_pk[5*SPX_WOTS_PK_BYTES];
    unsigned char block[SPX_N];
    unsigned char *blocks[KECCAK_WAY];
    ...
    double result;
    int i;
    uint64_t statex[KECCAK_WAY*25];

    randombytes(m, SPX_MLEN);
    randombytes(addr, SPX_ADDR_BYTES*4);
    ...
    check_overflow();
    reset_cpucycles();
    MEASURT("f1600x                      ", 1, keccakx_asm(statex));
    check_overflow();
    reset_cpucycles();
    MEASURT("thashx                      ", 1, thashx(blocks, (unsigned char const * const*)blocks,\
                                                      1, &ctx, (uint32_t*)addr));
    check_overflow();
    reset_cpucycles();
    ...
    check_overflow();
    reset_cpucycles();
    MEASURE("  - WOTS pk gen x (ideal).. ", (1 << SPX_TREE_HEIGHT) / KECCAK_WAY, wots_gen_pkx(wots_pk, &ctx, (uint32_t *) addr));
    check_overflow();
    reset_cpucycles();
    MEASURE("  - WOTS pk gen x (real)..  ", ((1 << SPX_TREE_HEIGHT) + (KECCAK_WAY-1)) / KECCAK_WAY, wots_gen_pkx(wots_pk, &ctx, (uint32_t *) addr));
    check_overflow();
    reset_cpucycles();
    ...
    check_overflow();
    reset_cpucycles();
    MEASURE("  - WOTS pk gen x (ideal).. ", SPX_D * (1 << SPX_TREE_HEIGHT) / KECCAK_WAY, wots_gen_pkx(wots_pk, &ctx, (uint32_t *) addr));
    check_overflow();
    reset_cpucycles();
    MEASURE("  - WOTS pk gen x (real)..  ", SPX_D * (((1 << SPX_TREE_HEIGHT) + (KECCAK_WAY-1)) / KECCAK_WAY), wots_gen_pkx(wots_pk, &ctx, (uint32_t *) addr));
    check_overflow();
    reset_cpucycles();
...
static void wots_gen_pkx(unsigned char *pk, const spx_ctx *ctx, uint32_t addr[8]) {
   struct leaf_info_x leaf;
   unsigned steps[ SPX_WOTS_LEN ] = { 0 };
   INITIALIZE_LEAF_INFO_X(leaf, addr, steps);
   wots_gen_leafx(pk, ctx, 0, &leaf);
}
